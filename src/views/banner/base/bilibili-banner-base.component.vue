            ...
            <img
              ...
              :style="{
                ...
                transform:
                  '' +
                  'translate(' +
                  (each.x == undefined ? 0 : imgNewData[i].currentX) +
                  'px, ' +
                  (each.y == undefined ? 0 : imgNewData[i].currentY) +
                  'px' +
                  ')' +
                  ' ' +
                  'rotate(' +
                  (each.r == undefined ? 0 : imgNewData[i].currentRotate) +
                  'deg)' +
                  ' ' +
                  'scale(' +
                  (each.s == undefined ? 1 : imgNewData[i].currentScale) +
                  ')',
...
const getLeftWidth = (): number => {
  ...
  if (props.maxLeftPosition) {
    ...
    let r =
      (props.imgData[i].newX - props.imgData[i].x) / props.imgData[i].bench;
    leftWidth =
      (props.imgData[i].width - containerWidth.value) / 2 - props.imgData[i].x;
...
const moveFunction = (i: number): void => {
  ...
  if (imgNewData.value[i].currentOpacity < 0) {
    // 透明度检测，在0~1范围内
